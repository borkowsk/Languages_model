...
{//!!!Niewiele mozna zrobic bo nie mozna tu jeszcze polegac na wirtualnych metodach klasy swiat
			...
			set_simulation_name("Languages_v1");
			set_bias_from_str("");
		}
...
generic_histogram_source*  ClassStat=new generic_histogram_source(Classif);//Zrodlo - histogram z klasyfikacji jezykow
...
/*iClusterSize1*/
pom=new sequence_graph(szer-49,14*char_height('X')+14,szer,15*char_height('X')+15,
							    3,Sources.make_series_info(
...
//-------------------------------------
{
	...
	if(first)
	  Log.GetStream()
		<<"\nPower range:"<<Log.separator()<<MinSila<<'-'<<MaxSila
