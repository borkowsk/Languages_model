...
void	_connect_flink_to(unsigned a,unsigned b,unsigned target_a,unsigned target_b);//Realizuje zadane połaczenie do (w załozeniu silniejszego) agenta target_a,target_b
void	_connect_far_links(double Percent);//Probuje przełączyc pewien procent dalekich linków

//Właściwe kroki różnych typów zaleznych od implementacji biasu
...
bool 				use_SW_links;//Czy używamy dalekich linków
...
jworld(size_t Width,	//Szerokosc torusa macierzy agentow
	  ...
	  bool i_use_SW_links=true,//Czy używamy dalekich linków
...
//------------------------------------------------------------------
void	make_default_visualisation();//Tworzy domyslne "lufciki" i umieszcza w swoim menagerze obszarów
