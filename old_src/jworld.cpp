...
jworld::jworld(size_t Width,		//Szerokosc torusa macierzy agentow
	  ...
	  bool i_use_SW_links,//Czy u¿ywamy dalekich linków
...
{//!!!Niewiele mozna zrobic bo nie mozna tu jeszcze polegac na wirtualnych metodach klasy swiat
			...
			_far_link::MyWorld=this;//Pod³¹czenie do celu odczytywania counterów przez tablice
				assert(Agenci.get_rect_geometry()->is_torus());//ZAWSZE TORUS. ZA£O¯ONE W ALGORYTMIE DALEKICH LINKÓW itd
			//Inicjowanie bezpoœrednich statystyk
			SW_dynamic_perc=0;
		}
...
//Tworzy z³o¿one serie (Ÿród³a) danych i definiuje log a potem
//O ILE JEST DOSTÊPNY Manager obszarów/lufcików
//umieszcza w nim domyslne wizualizacje - grafy
{
	//Metoda klasy bazowej - mo¿e nic nie robiæ, ale na wszelki wypadek
	world::make_default_visualisation(); //Obszar domyœlne - np obszar STATUSU
	bool WithGr=this->HasAreaMenager(); //Czy bêdzie tworzone wyjœcie graficzne?

//Indeksy serii potrzebne dalej dla grafów
//////////////////////////////////////////////
//Indeksy g³ównych serii
...
//JU¯ GOTOWE - MO¯NA EWENTUALNIE ODRYSOWAÆ
...
//-------------------------------------
{
	...
	if(use_SW_links)
		_connect_far_links(SW_startconnect_percent);//Probuje przy³¹czyc jakiœ procent dalekich linków przed startem
		
	first=0;//Koniec pierwszego wywolania //TYMCZASOWO!!! ???? HAHA!!! Nie mam pojêcia dlaczego...
}
...
void    jworld::_connect_far_links(double Percent)
{
	...
	unsigned Counter=0;
	for(unsigned i=0;i<N;i++)//N prób - choæ czasem mo¿e byæ podwójnie albo i wiEcej
	{
		unsigned const a=RANDOM(MyWidth);
		unsigned const b=RANDOM(MyWidth);
		//Losowo - proporcjonalnie do zasiêgu
 ...
 //			r=OdlSasiad+r*DRAND();//Zwykle losowanie w obrêbie promienia ale nie mniej ni¿ OdlSasiad
			r=OdlSasiad+r*DRAND()*DRAND()*DRAND();//Zagêszczone blisko losowanie w obrêbie promienia
			...
			if((a!=ta || b!=tb) && Agenci.filled(ta,tb)) //Nie ma sensu po³aczenie ze sob¹ i z pustym polem
			{
				//ALGORYTM BEZ LOSOWOSCI
#if 0
				if(Agenci.get(ta,tb).Power>=on.Power) //dobrowolne oddanie siê w protekcje lub czasem sojusz.
				{									 //Pomijaj¹c ingerencje zewnêtrzne po³¹czenie d¹¿y do 
													 //najsilniejszym agenta w zasiêgu r
													 
					double power_of_protector=0;
					if(FarLinks.get(a,b).a!=UINT_MAX)//Jest ju¿ jakiœ protektor
				...
				else //Próba przy³¹czenia - trzeba wysiudaæ aktualnego protektora
				{
					double power_of_protector=0;
					if(FarLinks.get(ta,tb).a!=UINT_MAX)//Jest jakiœ protektor
...
#else
				//ALGORYTM Z PROSTYM SZUMEM - SI£A ZREALIZOWANA MO¯E WYNOSIÆ OD 50 do 150% NOMINALNEJ
				if(Agenci.get(ta,tb).Power>=on.Power) //dobrowolne oddanie siê "on-ego" w protekcje 
													 //lub czasem zawarcie sojuszu (gdy si³y zblizone)
				{									 //Pomijaj¹c ingerencje zewnêtrzne  d¹¿y do po³¹czenia z
													 //najsilniejszym agenta w zasiêgu r
													 
					double power_of_protector=0;
					if(FarLinks.get(a,b).a!=UINT_MAX)//Gdy "on" ma ju¿ jakiegoœ protektora
						power_of_protector=Agenci.get(FarLinks.get(a,b).a,FarLinks.get(a,b).b).Power;
				...
				else //Próba przy³¹czenia sobie wasala - trzeba wysiudaæ aktualnego protektora
				{
					double power_of_protector=0;
					if(FarLinks.get(ta,tb).a!=UINT_MAX)//Jest jakiœ protektor
						power_of_protector=Agenci.get(FarLinks.get(ta,tb).a,FarLinks.get(ta,tb).b).Power;
